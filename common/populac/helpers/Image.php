...
class Image
{
    ...
    static function thumb($filename, $width = null, $height = null, $crop = true)
    {
        if($filename && is_file(($filename = Yii::getAlias('@webroot') . $filename)))
        {
            ...
            $thumbWebFile = Yii::$app->homeUrl . '/' . Upload::$UPLOADS_DIR . '/thumbs/' . $thumbName;
    ...
    static function thumbFrontend($filename, $width = null, $height = null, $crop = true)
    {
        /*if(!is_file($filename)) {
            $filename = '/admin' . $filename;
        }
        Yii::info($filename, 'filename');*/
        if($filename && is_file(($filename = str_replace('frontend', 'backend', Yii::getAlias('@webroot')) . $filename)))
        {
            $info = pathinfo($filename);
            $thumbName = $info['filename'] . '-' . md5( filemtime($filename) . (int)$width . (int)$height . (int)$crop ) . '.' . $info['extension'];
            $thumbFile = str_replace('frontend', 'backend', Yii::getAlias('@webroot')) . DIRECTORY_SEPARATOR . Upload::$UPLOADS_DIR . DIRECTORY_SEPARATOR . 'thumbs' . DIRECTORY_SEPARATOR . $thumbName;
            $thumbWebFile = '/admin/' . Upload::$UPLOADS_DIR . '/thumbs/' . $thumbName;
            //$thumbWebFile = '/' . Upload::$UPLOADS_DIR . '/thumbs/' . $thumbName;
            if(file_exists($thumbFile)){
                return $thumbWebFile;
            }
