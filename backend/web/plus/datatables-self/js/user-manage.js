$(function (){
	$('#form-add, #form-edit').formValidation({
		framework: 'bootstrap',
		excluded: ':disabled',
		icon: {
	...
	var _table = $table.dataTable($.extend(true,{},CONSTANT.DATA_TABLES.DEFAULT_OPTION, {
		ajax : function(data, callback, settings) {//ajax配置为function,手动调用异步查询
			...
			$.ajax({
		            type: "GET",
		            url: "/admin/plus/datatables-self/data.txt",
		            cache : false,	//禁用缓存
		            data: param,	//传入已封装的参数
		            dataType: "json",
...
//		_table.draw(false);
	...
	$("#btn-info-content-collapse").click(function(){
		...
		$("span",this).toggle();
		$("#user-view .populac-datatable-info-content").slideToggle("fast");
	});
...
var userManage = {
	...
	showItemDetail : function(item) {
		$("#user-view").show().siblings(".populac-datatable-info-block").hide();
		if (!item) {
	...
	addItemInit : function() {
		...
		$("#user-add").show().siblings(".populac-datatable-info-block").hide();
	},
	editItemInit : function(item) {
		...
		$("#user-edit").show().siblings(".populac-datatable-info-block").hide();
	},
